...
const uploadToCloudinary = (fileBuffer, folder) => {
  return new Promise((resolve, reject) => {
    const upload_stream = cloudinary.uploader.upload_stream({ folder: folder }, (error, result) => {
      if (result) {
        resolve(result);
      } else {
        reject(error);
...
const registerPostulante = async (req, res = response) => {
  ...
  try {
    ...
    const archivo_cv_result = await uploadToCloudinary(req.files['archivo_cv'][0].buffer, 'cv');
    const certificado_unico_laboral_result = await uploadToCloudinary(req.files['certificado_unico_laboral'][0].buffer, 'certificados');
